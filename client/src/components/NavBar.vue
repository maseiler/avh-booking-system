    ...
    <nav
      ...
      <div class="navbar-brand">
        <a class="navbar-item" @click="() => switchColorScheme(false, -1)">
          <font-awesome-icon icon="beer" size="lg" />
        </a>
      </div>
      ...
      <div id="navbarBasicExample" class="navbar-menu">
        <div class="navbar-start">
          <div class="navbar-item">
            <router-link class="button is-white" :to="'/'"> Home</router-link>
          </div>

          <div class="navbar-item">
            <router-link class="button is-white" :to="'/booking'">
              Booking
            </router-link>
          </div>

          <div class="navbar-item">
            <router-link class="button is-white" :to="'/statistics'">
              Statistics
            </router-link>
          </div>
        </div>
        ...
        <div class="navbar-end">
          <div
            v-if="!automaticThemeSwitching"
            class="navbar-item darkLightSwitch">
          ...
          <div
            class="navbar-item autoManualSwitch"
            title="Automatische Umschaltung aktivieren zwischen Dark/Light Theme">
            <font-awesome-icon icon="palette" />
            <input
              id="autoSwitch"
              v-model="automaticThemeSwitching"
              type="checkbox"
              class="cbSwitch"
              @click="automaticThemeSwitchClick" />
...
<script>
  export default {
    ...
    mounted: function () {
      ...
      this.automaticThemeSwitching =
        localStorage.getItem('automaticThemeSwitching') === 'true'
      if (localStorage.getItem('automaticThemeSwitching') == null) {
        this.automaticThemeSwitching = true
        localStorage.setItem(
          'automaticThemeSwitching',
          this.automaticThemeSwitching
        )
      }
    ...
    methods: {
      ...
      timeHandler() {
        ...
        let currentTimeInt = parseInt(
          currentTime.substr(16, 5).split(':').join('')
        )
